...
# Konfigurasi
# ==========================
st.set_page_config(page_title="Klasifikasi Wajah", page_icon="🙂", layout="centered")
st.title("🙂 Klasifikasi Wajah (Keras .h5)")
...
def debug_env():
    with st.expander("🔎 Debug (klik jika butuh)"):
        ...
        cands = glob.glob("**/*.h5", recursive=True)
        st.write("Model .h5 terdeteksi:", cands[:100])
...
def load_face_model_from_path(path: str):
    # Tambahkan custom_objects jika dipakai saat training
...
def get_face_model():
    ...
    up = st.file_uploader("Upload model wajah (.h5)", type=["h5"], key="upload_h5_face")
    if up is None:
        debug_env(); st.stop()
...
# UI
# ==========================
uploaded_img = st.file_uploader("Unggah Gambar Wajah (.jpg/.jpeg/.png)", type=["jpg","jpeg","png"])
if uploaded_img:
...
# pages/2_Klasifikasi_Kendaraan.py
import os, glob, tempfile
import numpy as np
import streamlit as st
from PIL import Image
...
st.set_page_config(page_title="Klasifikasi Kendaraan", page_icon="🚗", layout="centered")
st.title("🚗 Klasifikasi Kendaraan — Car vs Truck (YOLOv8)")
...
def debug_env():
    with st.expander("🔎 Debug (klik jika butuh)"):
        ...
        try:
            ...
            st.write("Gagal list dir:", e)
        cands = glob.glob("**/*.pt", recursive=True)
        st.write("Model .pt terdeteksi:", cands[:100])
...
def get_vehicle_model():
    ...
    up = st.file_uploader("Upload model kendaraan (.pt)", type=["pt"], key="upload_pt_vehicle")
    if up is None:
        debug_env(); st.stop()
...
# UI
# ==========================
uploaded_img = st.file_uploader("Unggah Gambar Kendaraan (.jpg/.jpeg/.png)", type=["jpg","jpeg","png"])
if uploaded_img:
    img = Image.open(uploaded_img).convert("RGB")
    st.image(img, caption="Gambar yang Diupload", use_container_width=True)
    ...
    if getattr(r0, "probs", None) is not None:
        ...
        if len(vec) == 2:
            # baca nama kelas index 0/1 → peta ke biner
            ...
            if b0 and b1 and b0 != b1:
                ...
                pred = "car" if p_car >= p_truck else "truck"
    ...
    # ===== CASE 2: model deteksi (boxes)
        ...
        if boxes is None or len(boxes) == 0:
            ...
            df["raw_name"] = df["cls"].map(lambda i: names.get(int(i), f"class_{int(i)}") if isinstance(names, dict)
                                           else names[int(i)] if names else f"class_{int(i)}")
